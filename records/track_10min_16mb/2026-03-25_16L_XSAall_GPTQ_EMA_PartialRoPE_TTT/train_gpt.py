...
class Hyperparameters:
    ...
    trigram_vocab_size = int(os.environ.get("TRIGRAM_VOCAB_SIZE", 10240))
    trigram_dim = int(os.environ.get("TRIGRAM_DIM", 64))

    ema_enabled = bool(int(os.environ.get("EMA_ENABLED", "1")))
    ema_decay = float(os.environ.get("EMA_DECAY", "0.999"))

    rope_partial_frac = float(os.environ.get("ROPE_PARTIAL_FRAC", "0.25"))  # fraction of head_dim to apply RoPE
    ...
    # Test-time training (LoRA adapters per document at eval)
...
def quantize_intN_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    """Per-row INT-N quantization with GPTQ-lite: grid search over clip ratios to minimize MSE."""
    t32 = t.float()
    if t32.ndim == 2:
        ...
        for ratio in _GPTQ_CLIP_RATIOS:
            ...
            err = (q.float() * scale_f32[:, None] - t32).pow(2).sum(dim=1)
            if best_err is None:
                best_q = q.clone()
...
def pack_nibbles(q: Tensor) -> Tensor:
    """Row-wise nibble packing: int8 values in [-8,7] → packed bytes (2 per byte).
    Input: 2D int8 tensor of shape (R, C) with values in [-8, 7].
    Output: int8 tensor of shape (R, ceil(C/2)).
    """
    assert q.dtype == torch.int8 and q.ndim == 2
    R, C = q.shape
    if C % 2 == 1:
        q = torch.cat([q, q.new_zeros(R, 1)], dim=1)
    u = q.to(torch.uint8) + 8           # shift [-8,7] → [0,15]
    return ((u[:, 0::2] << 4) | u[:, 1::2]).to(torch.int8)


def unpack_nibbles(packed: Tensor, orig_shape: tuple) -> Tensor:
    """Row-wise nibble unpacking: packed bytes → int8 values in [-8, 7].
    Input: int8 tensor of shape (R, ceil(C/2)).
    Output: int8 tensor of shape orig_shape == (R, C).
    """
    R, C = orig_shape
    u = packed.to(torch.uint8)
    out = torch.empty(R, packed.shape[1] * 2, dtype=torch.int8, device=packed.device)
    out[:, 0::2] = (u >> 4).to(torch.int8) - 8
    out[:, 1::2] = (u & 0x0F).to(torch.int8) - 8
    return out[:, :C]


def fake_quant_ste(w: Tensor, clip_range: int = 7) -> Tensor:
    """Per-row int4 fake-quantization for QAT using straight-through estimator.
    Forward: simulate int4 quantization (round to nearest, clamp to [-8, 7]).
    Backward: pass gradients through unchanged (STE).
    """
    row_max = w.detach().float().abs().amax(dim=1, keepdim=True)
    scale = (row_max / clip_range).clamp_min(1e-12).to(w.dtype)
    w_norm = w / scale
    w_rounded = w_norm.round().clamp(-(clip_range + 1), clip_range)
    w_q = w_norm + (w_rounded - w_norm).detach()   # STE: identity gradient
    return w_q * scale
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if info in ("passthrough", "passthrough_ctrl", "passthrough_fp16"):
            ...
            out[name] = t
            continue
        if isinstance(info, dict) and info.get("type") == "int4nibble":
            packed = result[name + ".q4n"]
            shape = tuple(info["shape"])
            q = unpack_nibbles(packed, shape)
            s = result[name + ".scale"]
            out[name] = (q.float() * s.float()[:, None]).to(orig_dtype)
            continue
...
def apply_rotary_emb(x: Tensor, cos: Tensor, sin: Tensor) -> Tensor:
    # Partial RoPE: cos/sin cover only rope_dim dims; remaining dims pass through unchanged
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, rope_base: float, qk_gain_init: float, rope_partial_frac: float = 0.25, kv_source: "CausalSelfAttention | None" = None):
        ...
        rope_dim = max(2, int(self.head_dim * rope_partial_frac) // 2 * 2)  # even, >= 2
        if rope_dim % 2 != 0:
    ...
    def forward(self, x: Tensor, q_delta=None, v_delta=None) -> Tensor:
        ...
        q = (self.c_q(x) + q_delta).reshape(bsz, seqlen, self.num_heads, self.head_dim).transpose(1, 2) if q_delta is not None else self.c_q(x).reshape(bsz, seqlen, self.num_heads, self.head_dim).transpose(1, 2)
        k = self.c_k(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(1, 2)
        v = (self.c_v(x) + v_delta).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(1, 2) if v_delta is not None else self.c_v(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(1, 2)
...
class TrigramHashEmbedding(nn.Module):
    ...
    def trigram_hash(self, tokens: Tensor) -> Tensor:
        ...
        # positions 0 and 1 have no full trigram context — use padding bucket (mod)
        out[..., 2:] = (73856 * t[..., 2:] ^ 19349 * t[..., 1:-1] ^ 83492 * t[..., :-2]) % mod
        return out.long()
...
class GPT(nn.Module):
    def __init__(
        ...
        trigram_dim: int = 64,
        rope_partial_frac: float = 0.25,
        xsa_num_layers: int = 0,
    ):
        super().__init__()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        if lora is not None:
            return F.cross_entropy(logits.float(), targets, reduction="none").reshape(bsz, seqlen)
...
class BatchedLinearLoRA(nn.Module):
    """Per-batch-element LoRA delta for a linear layer. ΔW = BA."""
...
class BatchedTTTLoRA(nn.Module):
    """LoRA adapters for all blocks: Q and V per block."""
...
def _build_ttt_optimizer(lora: BatchedTTTLoRA, args: "Hyperparameters") -> torch.optim.Optimizer:
...
def _find_docs(all_tokens: Tensor) -> list[tuple[int, int]]:
    """Return (start_offset, length+1) for each document, including the first token of the next doc."""
    bos_positions = (all_tokens == BOS_ID).nonzero(as_tuple=True)[0].tolist()
    docs = []
    for i, start in enumerate(bos_positions):
        end = bos_positions[i + 1] + 1 if i + 1 < len(bos_positions) else all_tokens.numel()
        if end - start >= 2:
...
def eval_val_ttt_lora(
    ...
    """Score-first TTT LoRA eval: accumulate BPB, then train LoRA on each chunk."""
    ...
    # Group by similar length to minimise padding waste
    ...
    for bi in range(0, len(rank_docs), batch_size):
        ...
        for ci in range(max_nc):
            chunk_end = (ci + 1) * chunk_size
            win_start_common = max(0, chunk_end - eval_seq_len)
            context_size = chunk_end - win_start_common
            ...
            for b in range(bsz):
                ...
                pl = pred_lens[b]
                nc = num_chunks[b]
                c_end = pl if ci == nc - 1 else (ci + 1) * chunk_size
                c_start = ci * chunk_size
                win_s = max(0, c_end - eval_seq_len)
                win_len = c_end - win_s
                chunk = all_tokens[ds + win_s : ds + win_s + win_len + 1].to(dtype=torch.int64, device=device)
                x[b, :win_len] = chunk[:-1]
                y[b, :win_len] = chunk[1:]
                co = c_start - win_s
                cl = c_end - c_start
            ...
            # Score: accumulate BPB before training on this chunk
            with torch.no_grad():
                for b in range(bsz):
                    if not active[b]:
                        continue
                    ...
                    prev = x[b, co : co + cl]
                    tb = base_bytes_lut[tgt].to(torch.float64)
                    tb = tb + (has_leading_space_lut[tgt] & ~is_boundary_token_lut[prev]).to(torch.float64)
                    loss_sum += lbl.sum()
                    byte_sum += tb.sum()
                    token_count += cl

            # Train: one Adam step on LoRA using chunk loss
            if needs_train:
                chunk_offset = ci * chunk_size - win_start_common
                mask = torch.tensor(
                    [float(ci < num_chunks[b] - 1) if active[b] else 0.0 for b in range(bsz)],
                    device=device,
                )
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        rope_partial_frac=args.rope_partial_frac,
    ...
    # EMA state: initialized from model at start of training
    ...
    # Final eval on int6-roundtripped weights
    torch.cuda.synchronize()
    t_qeval = time.perf_counter()
    if args.ttt_enabled:
        log0(f"final_eval_mode:ttt_lora rank:{args.ttt_lora_rank} lr:{args.ttt_lora_lr} chunk:{args.ttt_chunk_size} batch:{args.ttt_batch_size}")
