"""
SP4096 + Depth Recurrence (layers 4-5) + Parallel Residuals + MuonEq-R + QK-Gain 5.0
+ Self-Generated GPTQ Calibration + Score-First TTT + XSA-all

Stack on Kevin Clark's SP4096+MLPMult4+WD085 architecture, adding:
1. Depth recurrence: blocks 4 and 5 are repeated (shared weights, run twice) for free depth
2. Parallel residuals: attention and MLP outputs summed in parallel (not sequential)
   — same residual stream goes through both, outputs added together
3. MuonEq-R: normalize each gradient row before Newton-Schulz orthogonalization
4. QK-Gain initialized to 5.0 (sharper attention from step 1)
5. SP4096 tokenizer (4096 vocab, more tokens per byte = better BPB)
6. XSA-all: all blocks share K,V from block 0
7. Self-generated GPTQ calibration for activation-aware quantization
8. Score-first causal TTT on last 3 blocks (legal)
"""
...
class Hyperparameters:
    # Data paths: expects SP4096 tokenized data
    ...
    # QK-Gain: higher value → sharper attention; 5.0 matches top submissions
    ...
    recur_end = int(os.environ.get("RECUR_END", 6))  # exclusive
...
def main() -> None:
    ...
    # Self-generated GPTQ calibration
    log0("selfgen_calib: generating calibration sequences for activation-aware GPTQ...")
    torch.cuda.synchronize()
    ...
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, layer_acts=layer_acts)
    ...
    torch.cuda.synchronize()
    log0(
        f"final_int6_roundtrip val_loss:{base_val_loss:.8f} val_bpb:{base_val_bpb:.8f} "
